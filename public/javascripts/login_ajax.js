		...
		if(formElement.hasAttribute('action')){
			...
			if(window.FormData){
				...
				for(var nItem=0;nItem<formElement.elements.length;nItem++){
					if(formElement[nItem].nodeName.toUpperCase()==='INPUT'&&formElement[nItem].type.toUpperCase()==='TEXT'){
						search+=formElement[nItem].getAttribute('name')+'='+formElement[nItem].getAttribute('value');
					}
				}
				xhr.send(search);
			}
...
xhr.onreadystatechange=function(){
	if(xhr.readyState===XMLHttpRequest.DONE){
		if(xhr.status===200){
			...
			p_element.innerHTML=xhr.responseText;
